	...
	try:
		...
		if (now.minute % 15 == 0 or firstFlg) and last_minute != now.minute:
			...
			if flag["order"]["exist"]:
				...
				for order in orders:
					#if order['id'] == flag['order']['id']:
					if order['side'].upper() == flag["order"]["side"].upper() and order['price'] == flag["order"]["price"] and order['amount'] == flag["order"]["amount"]:
						bitflyer.cancel_order(
							symbol = product_code,
							id = order["id"],
							params = { "product_code" : product_code })
						
						print("未約定の注文をキャンセルしました。")
						break
				else:
					print("注文が約定しました！")
			...
			if buysell != '':
				...
				time.sleep(20)
				print("注文が完了しました。",parent_id)
				flag["order"]["exist"] = True
				flag["order"]["id"] = parent_id['parent_order_acceptance_id']
				flag["order"]["side"] = buysell.upper()
				flag["order"]["amount"] = amount
				flag["order"]["price"] = entry_price
