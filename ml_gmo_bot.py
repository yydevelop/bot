...
# gmocoin = ccxt.gmocoin()		 # 使用する取引所を記入
# gmocoin.apiKey = settings.gmoApiKey
# gmocoin.secret = settings.gmoSecret
...
# gmocoin = ccxt.gmocoin()
# gmocoin.apiKey = settings.apiKey
# gmocoin.secret = settings.secret
...
def get_price(min, before=0, after=0):
	...
	response = requests.get("https://api.cryptowat.ch/markets/gmocoin/btcfxjpy/ohlc",params)
	data = response.json()
...
# 			orders = gmocoin.fetch_open_orders(symbol = product_code, params = { "product_code" : product_code })
# 			if orders:
# 				for order in orders:
# 					gmocoin.cancel_order(
# 						symbol = product_code,
# 						id = order["id"],
...
# 			else:
# 				executions = gmocoin.private_get_getexecutions(params = { "product_code" : product_code })
# 				execution_buysell = ''
# 				execution_price = 0
...
# 			# 予測結果による売買
# 			positions = gmocoin.private_get_getpositions( params = { "product_code" : "FX_BTC_JPY" })
# 			pos = 0
...
api_key = "yirXnei+HGvRm9G9k9gC18OOQB2cUwqR"
secret = "1fSPh31TeYXQ36jTCetCngHN7gZUuPNqRxPts4dyobBpxZZFNu+GmheTAWlP3DxH"
